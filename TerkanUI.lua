...
local function tween(inst, props, time, style)
    local t = TweenService:Create(inst, TweenInfo.new(time or 0.14, style or Enum.EasingStyle.Quad, Enum.EasingDirection.Out), props)
...
function TerkanUI:Window(options)
    ...
    local footText = text(footer, self.Footer, 12, Theme.TextFaint, Enum.FontWeight.Medium, Enum.TextXAlignment.Center)
    ...
    table.insert(self.Conns, UserInputService.InputChanged:Connect(function(
        input
    )
    ...
    table.insert(self.Conns, UserInputService.InputEnded:Connect(function(
        input
    )
    ...
    table.insert(self.Conns, UserInputService.InputBegan:Connect(function(
        input,
        gp
    )
...
function Window:ApplyTheme(roles)
    ...
    local function protected(inst)
        ...
        while p and p ~= self.Gui do
            if p:GetAttribute('TerkanNoTheme') then
                return true
            end
...
local function decodeValue(kind, v)
    ...
    if kind == 'keybind' then
        ...
        if enumType and Enum[enumType] and Enum[enumType][name] then
...
local function fsAvailable()
    return typeof(writefile) == 'function' and typeof(readfile) == 'function' and typeof(isfile) == 'function' and typeof(isfolder) == 'function' and typeof(makefolder) == 'function' and typeof(listfiles) == 'function'
end
...
function Window:_configDir()
    ...
    pcall(function()
        ...
        if not isfolder(root .. '/configs') then
...
function Window:SaveConfig(name, overwrite)
    ...
    if overwrite and not exists then
    ...
    for flag, entry in pairs(self.Flags)do
        if not entry.NoSave then
            ...
            if ok and value ~= nil then
                data.flags[flag] = encodeValue(entry.Kind, value)
            end
            if ok and value == nil and entry.Kind == 'keybind' then
                data.flags[flag] = ''
            end
        end
    end
    ...
    for flag, raw in pairs(data.flags)do
        ...
        if entry then
            ...
            if value ~= nil or entry.Kind == 'keybind' then
                if pcall(entry.Set, value, false) then
                    applied += 1
                end
            end
        end
...
function Window:DeleteConfig(name)
    if not fsAvailable() or typeof(delfile) ~= 'function' then
        return false, 'Cannot delete files here'
    end
...
function Window:SetAutoload(name)
    ...
    local path = self.ConfigFolder .. '/autoload.txt'
    local ok = pcall(function()
        writefile(path, name and cleanName(name) or '')
    end)
...
function Tab:Section(name, side)
    ...
    if not isFirst then
        ...
        table.insert(self.Window.Accented, {
            sep,
            'BackgroundColor3',
        })
        new('UIPadding', {
...
function Section:Button(a, ...)
    ...
    local lbl = text(btn, o.Text or 'Button', 14, Theme.Text, Enum.FontWeight.Medium, Enum.TextXAlignment.Center)
...
function Section:Slider(a, ...)
    ...
    hit.InputBegan:Connect(function(input)
        ...
        win.ActiveDrag = function(position)
            setFromX(position.X)
        end
    end)
...
function Section:Dropdown(a, ...)
        ...
        if multi then
            ...
            for _, opt in ipairs(options)do
                if selected[opt] then
                    table.insert(picked, tostring(opt))
                end
            end
    ...
    local function openMenu()
        ...
        for index, opt in ipairs(options)do
            ...
            local itemLabel = text(item, tostring(opt), 13, isOn and Theme.Accent or Theme.TextDim, Enum.FontWeight.Regular)
        ...
        tween(menu, {
            Size = UDim2.fromOffset(math.floor(width), height),
        }, 0.14)
...
function Section:Keybind(a, ...)
    ...
    local keyLabel = text(btn, bindName(default), 12, Theme.Accent, Enum.FontWeight.Medium, Enum.TextXAlignment.Center)
    ...
    table.insert(win.Conns, UserInputService.InputBegan:Connect(function(
        input,
        gp
    )
        if listening then
    ...
    function api:Set(key, silent)
        setBind(key, not silent)
    end
    function api:IsDown()
        return bindIsDown(current)
    end
...
function Section:ColorPicker(a, ...)
    ...
    local function openMenu()
        ...
        if y + H > winH - 6 then
            y = (absPos.Y - rootPos.Y) / scale - H - 4
        end
...
function Window:GetNotifyPlaces()
    return NotifyPlaceNames
end
function Window:Notify(options)
    ...
    if n.Enabled == false and not options.Force then
        return
    end

    local duration = options.Duration or n.Duration
    local accent = options.Color or NOTIFY_KINDS[options.Kind or ''] or (n.UseThemeColor and Theme.Accent or n.Color)
    ...
    if body then
        tween(body, {TextTransparency = 0}, 0.2)
    end
