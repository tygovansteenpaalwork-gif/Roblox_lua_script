--[[
    ...
    Shoots what is under the cursor, with delay, max distance, team check and a status line.
    ...
    Run it:   loadstring(game:HttpGet("https://raw.githubusercontent.com/tygovansteenpaalwork-gif/Roblox_lua_script/main/features/triggerbot.lua"))()
--]]
...
local ctx = Core({ Name = "triggerbot", Title = "Triggerbot" })

local BIND, C, FIRE_METHODS, Players, Ready, RunService = ctx.BIND, ctx.C, ctx.FIRE_METHODS, ctx.Players, ctx.Ready, ctx.RunService
local U, UserInputService, cam, charOf, connect, cursorOverMenu = ctx.U, ctx.UserInputService, ctx.cam, ctx.charOf, ctx.connect, ctx.cursorOverMenu
local dropdown, fireWeapon, keybind, lp, overlay, rayParams = ctx.dropdown, ctx.fireWeapon, ctx.keybind, ctx.lp, ctx.overlay, ctx.rayParams
local renderLast, sameTeam, slider, toggle, viewportCenter, win = ctx.renderLast, ctx.sameTeam, ctx.slider, ctx.toggle, ctx.viewportCenter, ctx.win
