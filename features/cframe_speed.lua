--[[
    CFrame Movement  -  cframe_speed.lua
    CFrame Speed, CFrame Fly and Dash: move the character by changing its CFrame.
    ...
    Run it:   loadstring(game:HttpGet("https://raw.githubusercontent.com/tygovansteenpaalwork-gif/Roblox_lua_script/main/features/cframe_speed.lua"))()
--]]
...
local ctx = Core({ Name = "cframe_speed", Title = "CFrame Movement" })

local BIND, C, Ready, U, UserInputService, cam = ctx.BIND, ctx.C, ctx.Ready, ctx.U, ctx.UserInputService, ctx.cam
local keybind, lp, myHumanoid, renderLast, slider, toggle = ctx.keybind, ctx.lp, ctx.myHumanoid, ctx.renderLast, ctx.slider, ctx.toggle
local win = ctx.win

local moveTab = win:Tab("CFrame Movement")
